var Baddy = function(name, color, position, direction) {
...
Baddy.prototype.decelerate = function (distance) {
    var min = -1;
...
Baddy.prototype.displayInfo = function () {
    jQuery('#'+this.name+' >.life').text(this.life);
}

Baddy.prototype.turnRight = function (angle) {
    this.direction -= angle;
    this.graphic.rotateOnAxis(new THREE.Vector3(0,0,1), -angle);
};

Baddy.prototype.turnLeft = function (angle) {
    this.direction += angle;
    this.graphic.rotateOnAxis(new THREE.Vector3(0,0,1), angle);
};
